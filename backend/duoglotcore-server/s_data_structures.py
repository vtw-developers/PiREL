...
For manipulating translation rules, classes in s_post_process_translation_rule.py are used.
s_post_process_translation_rule.py data structures are a bit different than DuoGlot-style AST's.
'''
...
class DuoGlotNode():
  ...
  def get_terminal_tokens(self) -> List[str]:
    '''
...
class PirelTree:
  ...
  def update_terminal_token_at_templatizable_node(self, node_id: int, new_text: str) -> None:
    '''
    Given a node_id of a node that can be templatized and update node_text
    attributes for all relevant nodes.

    This method is used in s_post_process_llm_gen_code module.

    POST: self is mutated
    '''

    def _get_new_parent_text(node: 'PirelNode', old_node_text: str, new_node_text: str) -> str:
      parent = node.get_parent()
      parent_text = parent.get_text()

      # 1 find the starting index to search old_node_text
      node_idx = node.get_index_as_child()

      # 1.1 iterate over siblings to the left, not including the node itself
      search_start_idx = 0
      for i in range(node_idx):
        sibling_left = parent.get_children()[i]
        sibling_text = sibling_left.get_text()
        sibling_text_idx = parent_text.find(sibling_text, search_start_idx)
        assert sibling_text_idx != -1, 'something is wrong, debugging needed'
        search_start_idx = sibling_text_idx + len(sibling_text)

      # 2 starting index of text of `node`
      node_text_idx = parent_text.find(old_node_text, search_start_idx)
      assert node_text_idx != -1, 'something is wrong, debugging needed'

      # 3 replace old node text with new one
      new_parent_text = parent_text[:node_text_idx] + new_node_text + parent_text[node_text_idx + len(old_node_text):]
      return new_parent_text

    node = self.get_node_with_id(node_id)
    assert node._PY_can_be_templatized(), 'The node cannot be templatized'

    # 1 find the terminal node under `node`
    terminals : List[PirelNode] = []
    def _visit_collect_terminals(node: 'PirelNode'):
      nonlocal terminals
      if node.is_terminal():
        terminals.append(node)
    self._pre_order(node, _visit_collect_terminals)
    # sanity check
    assert len(terminals) == 1, 'templatizable nodes have exactly one terminal node under'
    terminal_node = terminals[0]

    # 2 update the text of the terminal node
    old_node_text = terminal_node.get_text()
    terminal_node.set_text(new_text)
    terminal_node.set_type(new_text)

    # 3 update the texts of other relevant nodes (up the tree)
    # first iteration is manual
    cursor_node = terminal_node
    old_parent_text = cursor_node.get_parent().get_text()
    new_parent_text = _get_new_parent_text(cursor_node, old_node_text, new_text)

    cursor_node = cursor_node.get_parent()
    cursor_node.set_text(new_parent_text)
    # next iterations are looped
    while cursor_node.get_parent() is not None:
      old_current_node_text = old_parent_text
      old_parent_text = cursor_node.get_parent().get_text()
      new_parent_text = _get_new_parent_text(cursor_node, old_current_node_text, new_parent_text)
...
class PirelNode():
  ...
  def is_type_isomorphic_to(self, other_node: 'PirelNode') -> bool:
    ...
    NOTE
    1. This method is used in s_extract_templates.extract_templates() to filter out
    invalid templates.
  ...
  def _PY_has_parent_type__call(self) -> bool:
    return self.has_parent() and self.parent.get_type() == 'py.call'

  def _PY_has_type__string_content(self) -> bool:
    return self.node_type == 'py.string_content'

  def _PY_has_type__comment(self) -> bool:
    return self.node_type == 'py.comment'

  def _PY_can_be_templatized(self) -> bool:
    if self.is_terminal():
      return False
    if self.has_single_terminal_child():
      if self._PY_has_parent_type__call():
        return False
      if self._PY_has_type__string_content():
        return False
      if self._PY_has_type__comment():
        return False
      return True
    return False
...
def _test_pirel_tree_node():
  with open('temporary_full_ast.json') as fin:
    ast = json.loads(fin.read())
  with open('temporary_full_ast_text.json') as fin:
    astt = json.loads(fin.read())
  # tree = DuoGlotTree(ast)
  # tree.debug_print()
  ttree = PirelTree(astt)
  ttree.debug_print()


def _test_pattern_tree_node():
  path = '../../data/logs-rule-inference/target-unified-pattern.json'
  path = '../../data/logs-rule-inference/source-unified-pattern.json'
  with open(path) as fin:
    pattern = json.loads(fin.read())
  tree = PatternTree(pattern, 'py')
  tree.debug_print()
  node1 = tree.get_node_with_type('".1"')
  node2 = tree.get_node_with_type('".2"')
  node3 = tree.get_node_with_type('"*3"')
  print(node1.get_path_to_root_source())
  print(node2.get_path_to_root_source())
  print(node3.get_path_to_root_source())


if __name__ == '__main__':
  _test_pattern_tree_node()
