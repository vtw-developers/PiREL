...
def run_step_trans(bench_prefix, is_prepend_bef_test, gold_cleanup_func, source_path, testscript_list, gold_path, is_allinone, PY2JS_SCRIPT_PATH, **other_args):
  skipcount = 0
  for file in sorted(list(os.listdir(source_path))):
    ...
    transgold_savepath = os.path.join(gold_path, jsfile)
    save_content(gold_savepath, gold_code)
    time1 = timeit.default_timer()
    command = f"python2 {PY2JS_SCRIPT_PATH} {gold_savepath} " + \
      f"--output {transgold_savepath} " + \
      f">{os.path.join(gold_path, file + '.out')} 2>{os.path.join(gold_path, file + '.err')}"
...
def run_step_fill(bench_prefix, PY2JS_PATH, source_path, testscript_list, gold_path, testscript_path, combined_savepath, **other_args):
  skipcount = 0
  errorcount = 0
  libcode = read_content(f"{PY2JS_PATH}/py-builtins.js")
  for file in sorted(list(os.listdir(source_path))):
    if not file.startswith(bench_prefix): continue
    jsfile = file.replace(".py", ".js")
    filedir = file.replace(".py", "")
    ...
    libconcat_content = "/////libcode-start/////\n" + libcode +  "/////libcode-end/////\n" + goldtrans_content
    ...
    combined_testcode = test_content.replace("//TRANSlATED_PLACEHOLDER_NO_OUTPUT_EXPECTED", libconcat_content)
...
def run_step_stat(bench_prefix, is_showerr, is_ext, additionallogpath, source_path, testscript_list, runoutputpath, **other_args):
  ...
  solved_set_fname = f"./eval_py2js_solved_set.ext.json" if is_ext else f"./eval_py2js_solved_set.json"
...
def run_step(step_name, PY2JS_SCRIPT_PATH, PY2JS_PATH, bench_prefix, is_prepend_bef_test, is_showerr, is_ext, is_allinone, gold_cleanup_func, source_path, gold_path, combined_savepath, runoutputpath, additionallogpath, testscript_path, testscript_list):
  if step_name in STEP_FUNC_DICT:
    ...
    print(f"\n\n------------------------------- eval_py2js Run steps {step_name} argsdict: -------------------------------")
